...
export async function POST(request: Request) {
  ...
  // Create the store (unapproved). Arranca con la prueba gratis corriendo
  // (suscripcion_hasta): si activa el modo citas, el gating empieza al vencer.
  // Los días salen de TRIAL_DIAS y no de un INTERVAL fijo, para que cambiar la
  // duración de la prueba sea un solo número y no una cacería por el repo.
  await query(
    "INSERT INTO puestos (id, nombre, descripcion, ubicacion, aprobado, telefono_contacto, lat, lng, ciudad, suscripcion_hasta) VALUES ($1, $2, $3, $4, false, $5, $6, $7, $8, NOW() + make_interval(days => $9))",
    [puestoId, nombre_tienda, descripcion || null, direccion || null, tel, lat || null, lng || null, ciudadValida, TRIAL_DIAS]
  );
