...
export async function GET(_req: Request, { params }: { params: Promise<{ token: string }> }) {
  ...
  const items = await query<{
    ...
    subtotal: string; estado_cocina: string; variante_nombre: string | null; modificadores: unknown;
    ...
    `SELECT pi.id, COALESCE(pr.nombre, pi.producto_nombre, 'Producto') AS producto_nombre,
            ...
            pi.variante_nombre, pi.modificadores, pi.notas, p.created_at
     FROM pedidos p
  ...
  const parsed = items.map((i) => ({
    ...
    // La nota vuelve al comensal para que compruebe que se mandó bien.
    notas: i.notas,
  }));
